...
def parse_record_02_submissions(raw_df):
    ...
    for _, record in raw_df.iterrows():
        ...
        # 3. Supplier Name (Check 'Other' if selected)
        sup_main = str(
        ...
        # 6. Critical Limits: Packaging in good condition / in date / correctly labelled
        limits_raw = str(
        ...
        # Excursion validation: if temperature is required and > 5.0°C (or packaging bad)
...
def render_record_02_view(raw_df, selected_day_str, start_date, end_date):
    ...
    # -------------------------------------------------------------
    # TAB 2: 7 FULL-WIDTH DATE COLUMNS (NO SCOPE ON LEFT)
    # -------------------------------------------------------------
    with tab_matrix:
        ...
        # 2. Daily Receiving Card directly below date
        for i, d in enumerate(page_dates):
